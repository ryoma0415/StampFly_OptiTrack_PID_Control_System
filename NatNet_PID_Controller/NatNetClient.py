...
# Unpack Mocap Data Functions
        ...
        while not stop():
            ...
            if len(buffer_list[buffer_list_in_use_index]) > 0:
                ...
                if message_id == self.NAT_FRAMEOFDATA:
                    # Only apply modulo logic if print_level is greater than 1
                    # If print_level is 0, keep it as 0
                    if print_level > 1:
                        if (message_id_dict[tmp_str] % print_level) == 0:
                            print_level = 1
                        else:
                            print_level = 0
                    # If print_level is 0 or 1, keep it unchanged
    ...
    def __process_message(self, data: bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA:
            ...
            offset += offset_tmp
            # Disabled MoCap Frame output to keep console clean
            # if print_level >= 1:
